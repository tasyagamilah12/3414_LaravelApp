    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex justify-between items-end mb-12">
            ...
            <div class="mb-8 flex gap-4 justify-center">
                ...
                <!-- Melakukan iterasi nama Tab Kategori dinamis saat jumla data bertambah -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($events as $event)
                    <div
                        class="group bg-white rounded-3xl border borderslate-100 shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden">
                        <div class="relative overflow-hidden aspect-[3/4]">
                            ...
                            $imageMap = [
                                ...
                                $image = $imageMap[$slug] ?? 'concert.png'; // fallback ke concert.png
                                @endphp
                                <img src="{{ asset('assets/' . $image) }}" alt="{{ $event->title }}"
                                class="w-full h-full object-cover grouphover:scale-110 transition-transform duration-500">
                            <div
                                class="absolute top-4 left-4 px-3 py-1 bgwhite/90 backdrop-blur rounded-lg text-xs font-bold uppercase text-indigo-600">
                                {{ $event->category->name }}</div>
                        </div>
                        <div class="p-6">
                            <h3 class="text-xl font-bold mb-2 grouphover:text-indigo-600 transition">{{ $event->title }}
                            </h3>
                            <div class="flex items-center gap-2 text-slate500 text-sm mb-4">
                            ...
                            <div class="flex justify-between items-center pt-4 border-t">
                                <span class="text-2xl font-black textindigo-600">Rp
                                    {{ number_format($event->price, 0, ',', '.') }}</span>
                                <a href="{{ url('event/1') }}"
                                    class="px-5 py-2 bg-indigo-50 textindigo-600 rounded-xl font-bold hover:bg-indigo-600 hover:text-white transition">Lihat
                                    Detail</a>
                            </div>
                        </div>
                    </div>
